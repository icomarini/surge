...
{
    const auto x = randomFloat();
    const auto y = randomFloat();
    const auto z = randomFloat();
    const auto w = randomFloat();
    const auto n = std::sqrt(x * x + y * y + z * z + w * w);
    assert(n > 0);
    return std::make_tuple(x / n, y / n, z / n, w / n);
}
...
public:
    ...
    HelloTriangleApplication(const std::map<std::string, std::filesystem::path>& resources)
        ...
        , command {}
        , presenter { command }
        , defaults { command, resources.at("root") }
        , skybox { command, resources.at("shaders"), resources.at("skyboxTexture") }
        , assets { createAssets(command, resources) }
        , renderer { command, resources.at("shaders"), assets }
        , overlay { command, resources.at("shaders"), userInteraction, assets }
    // , shadowMap { command, presenter.renderPass, loadedTexture, loadedModel, shaders }
    // , scene { command, presenter.renderPass, GLTFModel { loadedGltfModel }, shaders }
    {
    }
    ...
    void run()
    {
        ...
        while (!surge::context().exit())
        {
            if (elapsedTime > 1.0 / 144.0)
            {
                ...
                render(presenter, userInteraction, skybox, /*vikingRoom,*/ renderer, /*shadowMap, scene,*/
                       overlay);
...
private:
    ...
    {
        const auto [extent, frameBuffer, commandBuffer] = presenter.acquire(command.renderPass);

        (pipelines.update(extent, ui), ...);

        command.record(extent, frameBuffer, commandBuffer, pipelines...);
...
private:
                                                  ...
                                                  const std::map<std::string, std::filesystem::path>& resources)
    {
        ...
        // activate oaktree
        auto& oaktree        = assets.at(0);
        oaktree.state.active = true;
        for (auto& mesh : oaktree.meshes)
        {
        ...
        for (auto& node : oaktree.mainScene().nodes)
        {
...
int main()
{
    try
    {
        ...
        [[maybe_unused]] const std::map<std::string, std::filesystem::path> resources {
            ...
            { "shaders", rootPath / "build/shaders" },
            ...
            { "buggy", "/home/ico/projects/extern/Vulkan/assets/models/gltf/glTF-Embedded/Buggy.gltf" },
        ...
        app.run();
        std::cout << "\033[1;37m[surge of INFO]\033[0m The surge of urge to purge terminated" << std::endl;
    }
...
int main(int, char**)
{
    ...
    [[maybe_unused]] ImFont* font =
        io.Fonts->AddFontFromFileTTF("/home/ico/projects/extern/imgui/misc/fonts/DroidSans.ttf", 25);
    assert(font);
